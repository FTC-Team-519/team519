 ...
 */
@TeleOp
public class FRRTeleopAlt extends ActiveOpMode {
    ...
    private int desiredLiftPosition = GROUND_HEIGHT;
    ...
    if (!driver.x && !driver.b) {
        ...
        if (gunner.a) {
            ...
            if (!gunner.b) {
                ...
                lift.setTargetPosition(alterLiftDesiredPosition());
            } else if (gunner.b)

            {
                lift.setPower(slowLiftPower());
                lift.setTargetPosition(slowLiftDesiredPosition());
            }
        }
    ...
    private int alterLiftDesiredPosition() {
        int alterAmount = (int)(-gY * 10.0);
        if (alterAmount != 0) {
            desiredLiftPosition += alterAmount;
        }
        else {
            desiredLiftPosition = lift.getCurrentPosition();
        }

        return desiredLiftPosition;
    }

    private int slowLiftDesiredPosition() {
        int alterAmount = (int)(-gY * 5.0);
        if (alterAmount != 0) {
            desiredLiftPosition += alterAmount;
        }
        else {
